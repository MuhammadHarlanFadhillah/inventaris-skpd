<?php 
// MULAI BUFFERING (PENTING BIAR GAK BLANK PAGE)
ob_start();
...
// ===============================================================
// LOGIKA SIMPAN (DITARUH PALING ATAS)
// ===============================================================
if (isset($_POST['simpan_transaksi'])) {
    ...
    // DEFAULT ID SKPD (Pastikan di DB kolom ini boleh string/varchar)
    // Kalau error "Foreign Key", ubah ini jadi NULL (tanpa kutip)
    $id_skpd = "SKPD-01"; 

    $error = false;

    // VALIDASI STOK (Khusus Keluar)
    if ($jenis == 'KELUAR') {
        ...
        if ($stok_db < $jumlah) {
            echo "<script>alert('⛔ TRANSAKSI DITOLAK!\\nStok $nama_db sisa $stok_db.'); window.history.back();</script>";
            $error = true;
        }
    }
    ...
    if (!$error) {
        ...
        if (mysqli_query($conn, $q_header)) {
            ...
            if (mysqli_query($conn, $q_detail)) {
                ...
                // D. SUKSES & REDIRECT (PAKAI JS BIAR AMAN)
                echo "<script>
                ...
                // Gagal Detail - Hapus Header biar bersih
                mysqli_query($conn, "DELETE FROM stok_persediaan WHERE id_stok='$id_stok'");
                die("<div class='alert alert-danger'><h3>Gagal Simpan Detail!</h3>Error: " . mysqli_error($conn) . "</div>");
            }
        } else {
            die("<div class='alert alert-danger'><h3>Gagal Simpan Header!</h3>Error: " . mysqli_error($conn) . "</div>");
        }
    }
}
?>
                ...
                <form method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="id_barang" class="form-select bg-light" required>
                            ...
                            <?php
                            $b = mysqli_query($conn, "SELECT id_barang, nama_barang, stok_akhir, satuan FROM barang ORDER BY nama_barang ASC");
                            while ($row = mysqli_fetch_assoc($b)) {
                                // Paksa ambil data dengan berbagai kemungkinan key
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="jumlah" class="form-control form-control-lg" min="1" required>
                    </div>
